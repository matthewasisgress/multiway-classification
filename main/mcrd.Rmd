---
title: "Multiway classification with R package **cpfa**: An application to four 
       public datasets"
...
subset to 2000 images from the original training set. The images include 980 
images of the digit 2 and 1020 images of the digit 3.
...
We plot an example of the digit 2 and of the digit 3.
...
We load R package **cpfa** and initialize the multiway model. First, the data 
...
Finally, we use the function `pficpfa`. The function calculates permutation 
feature importance (PFI; Breiman, 2001), which can be used to compare features 
to one another with respect to their individual contributions to classification
performance. For any one feature, its values are shuffled across observations,
breaking its relationship to the outcome; importance is the change in a 
classification performance measure observed before and after the shuffle. In 
this case, the features are the predicted (i.e., test set) classification mode 
component weights for each replication. The function re-shuffles `nshuffles` 
times and averages the results across shuffles. For regular PFI, we can set the 
function argument to `type = marginal`. 

However, for correlated features, a conditional variant of PFI (Huang, 2025; 
O'Gorman, 2005) can be used by setting `type = conditional`. This model-based 
conditional PFI uses a conditioning model (i.e., ridge regression or random 
forest) to predict a feature from the other features; and the model's residuals 
are shuffled and added back to the fitted values, preserving the feature's 
dependence on the other features. Moreover, by setting `safealign = TRUE`, 
split replications can be screened per component model using the Tucker 
congruence coefficients (TCCs) in `output$tccb`: splits whose `safealign.stat` 
value does not exceed `safealign.threshold` are excluded. We demonstrate a 
conditional PFI calculation below, using ridge regression as the conditioning 
model by setting `conditional.model = "ridge"`, and using a per split 
minimum (i.e., via `safealign.stat = "min"`) TCC value of 0.9 (i.e., via
...
# calculate conditional PFI for MNIST
pfistats <- pficpfa(object = outputR, nshuffles = 30, type = "conditional", 
                    conditional.model = "ridge", safealign = TRUE, 
                    safealign.stat = "min", safealign.threshold = 0.9,
                    parallel = FALSE)

# examine conditional PFI for accuracy for the two-component model
pfistats[which((pfistats$nfac == 2) & (pfistats$metric == "acc")), ]
```

We see that accuracy performance is driven by the second component; however, the 
first component offers a slight contribution. This pattern is present for both 
PLR and RF. The `nvalid` column indicates that all five of the replications were 
used for PFI calculations. If the `plotcpfa` interpretation of the second 
component is valid, this conditional PFI result suggests that the presence of 
the closed circle in the digit 2, as opposed to the the open or half circle seen 
in the digit 3, is largely responsible for either classifier's ability to 
distinguish between the digits in this two-component model.
...
2026) to download the dataset. We subset to 1000 images from the training set. 
...
Next, we initialize classification methods. First, we use 
`method = c("SVM")` to use support vector machine (SVM). Second, we specify that 
the problem is a multiclass classification 
problem by setting `family <- "multinomial"`. Third, we use 
...
parameters gamma (SVM) and cost (SVM), wrapping them into `parameters`.
...
Accuracy values are between 0.92 and 0.938. The three-component Parafac2 model 
with the SVM classifier performed best. In addition, average optimal tuning 
...
For the second component, weights are stronger between levels 6 and 9 and 
between 22 and 25. This second component might be capturing zero values to the 
left and right of images, contrasting them with non-zero values in the middle. 
Interestingly, we see two negative bands between levels 11 and 14 and between 16 
and 19; however, level 15 is less negative. Level 15 might correspond to the 
empty space between the trouser legs while the negative bands might represent 
non-zero values in the legs. If so, this component might help distinguish 
trousers from tops or sandals.

For the third component, weights are stronger between levels 6 to 23. This 
...
the top and trouser generally do not; so this third component might help 
distinguish the top and the trouser from the sandal.

Overall, the first two components correspond closely to one another, indicating
that the three-component solution might represent an overfactoring of the data.
The two-component solution might offer a better tradeoff between parsimony and 
classification performance, especially given how close the two solutions match
one another in accuracy. Beyond that, the one-component solution might be worth
considering as well.
...
The three-component Parafac model with the RDA classifier performed best. 
In addition, average optimal tuning parameters are available for each 
classifier. 
...
of visualizing each blood vessel object. However, the weights across all three
modes clearly show that the three-component solution is likely an overfactoring
and that it would be prudent to examine the weights for the two-component or 
even one-component solution. Likely, one or two components would be enough for
these data. This case demonstrates how the package can be a useful exploratory
tool for examining tradeoffs between optimal classification performance, 
parsimony, and interpretation. For more information about this dataset, consider 
exploring Yang, Xia, Kin, and Igarashi (2020); Yang et al. (2023); and Yang, 
Shi, and Ni (2021).
...
spleen (10). As with VesselMNIST3D, we download OrganMNIST3D manually from 
<https://medmnist.com/>, which gives us a .npz file. After changing the name
...
Next, we initialize classification methods. First, we use 
`method = c("RF", "SVM", "GBM")` to use RF, SVM, and GBM. Second, we specify 
that the problem is multiclass, setting `family <- "multinomial"`. Third, we use 
10-fold CV in our inner training, setting `nfolds <- 10`; and fourth, we set 
`nrep <- 5` to perform five outer train-test splits. Fifth, we set 
`ratio <- 0.9`. Finally, we specify ranges for tuning parameters the number 
of trees (RF), node size (RF), gamma (SVM), cost (SVM), eta (GBM), maximum depth
(GBM), subsample (GBM), and the number of rounds (GBM), wrapping them into the 
list called `parameters`.
...
The four-component Parafac2 model with the RF classifier performed best. In 
addition, average optimal tuning parameters are available for each classifier. 
...
We omit interpretations of the four components, given the difficulty of 
visualizing each organ object. Additional work would need to be performed to 
investigate estimated B weights and C weights to interpret components further.
