...
Accuracy values are between 0.9 and 0.922. The three-component Parafac2 model 
...
Next, we initialize classification methods. First, we use 
`method = c("RF", "SVM")` to use RF and SVM. Second, we specify that the problem 
...
gamma (SVM), and cost (SVM), wrapping them into the list called `parameters`.
...
Accuracy values are between 0.50 and 0.61. The four-component Parafac2 model 
...
Examining medians reveals differentiation among classes across the components.
